...
Triple braces do not escape HTML. Use them only for trusted HTML generated by the builder, such as `content`, `slots.sidebar`, `slots.aside`, and `slots.prevNext`.
...
If the new layout is still a documentation page, copy and adjust this structure. If the new layout is a homepage or marketing page, keep the shared header and design the page body yourself.
