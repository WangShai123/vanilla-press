...
function normalizeThemeDefaultValue(defaults: ThemeDefaultInput, key: ThemeDefaultKey): string {
  const value = typeof defaults?.[key] === "string" ? defaults[key].trim() : "";
  return value || DEFAULT_THEME_DEFAULT[key];
}

export function normalizeThemeDefault(defaults: unknown = {}): ThemeDefaultConfig {
