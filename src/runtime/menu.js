...
function menuItemIsActive(item, page) {
  const href = normalizeRel(item.href || item.url || '');
  const rel = normalizeRel(page.rel || '');
  if (href && href === rel) return true;
  return Array.isArray(item.children) && item.children.some((child) => menuItemIsActive(child, page));
}
...
function toMenuItems(items = [], page = {}, i18n) {
  return items.map((item, index) => {
    const children = Array.isArray(item.children) ? item.children : [];
    const classes = Array.isArray(item.classes) ? [...item.classes] : [];
    if (menuItemIsActive(item, page)) classes.push('current-menu-item');

    return {
      id: item.id || `${index}-${slugifyMenu(localize(item.i18n || item.label || item.title, i18n))}`,
      title: localize(item.i18n || item.label || item.title, i18n),
      url: item.href || item.url ? relativeAsset(page.rel, item.href || item.url) : '#',
      target: item.target,
      classes,
      children: toMenuItems(children, page, i18n),
    };
  });
}

function renderMenuItem(item, page, i18n) {
  const li = document.createElement('li');
  const children = Array.isArray(item.children) ? item.children : [];
  const active = menuItemIsActive(item, page);
  ...
  link.href = item.href || item.url ? relativeAsset(page.rel, item.href || item.url) : '#';
  ...
  if (children.length) {
    ...
    children.forEach((child) => subMenu.append(renderMenuItem(child, page, i18n)));
    li.append(subMenu);
  }
...
export function initHeaderMenu(menuItems = [], page = {}, i18n) {
  ...
  createEffect(() => {
    ...
    list.className = 'menu';
    menuItems.forEach((item) => list.append(renderMenuItem(item, page, i18n)));
    nav.append(list);
  });
...
export function initMobileHeader(menuItems = [], page = {}, i18n) {
  ...
  const drawer = createOffcanvas({
    ...
    onShow: () => {
      ...
      menu = new Menu({
        type: 'mobile',
        items: toMenuItems(menuItems, page, i18n),
      }).build();
...
function renderSidebarItem(item, page, i18n) {
  const children = Array.isArray(item.children) ? item.children : [];
  ...
  if (!children.length) {
    wrapper.href = item.href || item.url ? relativeAsset(page.rel, item.href || item.url) : '#';
  ...
  children.forEach((child) => list.append(renderSidebarItem(child, page, i18n)));
  wrapper.append(list);
  return wrapper;
}

function renderSidebar(sidebarItems = [], page = {}, i18n) {
  const nav = document.createElement('nav');
  nav.className = 'doc-nav';
  nav.setAttribute('aria-label', '文档导航');
  sidebarItems.forEach((item) => nav.append(renderSidebarItem(item, page, i18n)));
  return nav;
}

export function initSidebar(sidebarItems = [], page = {}, i18n) {
...
export function initMobileSecondary(sidebarItems = [], page = {}, i18n) {
  ...
  sidebarPanel.append(renderSidebar(sidebarItems, page, i18n));
